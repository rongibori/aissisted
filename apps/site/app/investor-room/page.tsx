...
/**
 * Investor Room v6 — conversion system · scored pipeline · world-class posture.
 *
 * v6 upgrades v5 from "cinematic room with capture" into a full conversion
 * system: every capture is scored hot/warm/cold, every intent fires two
 * transactional emails (investor confirmation + founder notification via
 * Resend), and a hidden /_/pipeline dashboard renders the live Airtable
 * pipeline behind an ADMIN_PIPELINE_TOKEN gate.
 *
 * New in v6:
 *   · Investor scoring     · deterministic hot/warm/cold (lib/investor-scoring)
 *                            pushed to HubSpot + Airtable on every capture.
 *   · Resend email fan-out · investor confirmation + founder alert per lead.
 *   · Allocation urgency   · GET /api/investor/allocation-status surfaces
 *                            seats remaining above the HardCTA — renders
 *                            nothing unless env seats_total/seats_filled are
 *                            set. No fabricated scarcity.
 *   · Founder calendar     · NEXT_PUBLIC_FOUNDER_CALENDAR_URL opens direct
 *                            booking from inside the modal for
 *                            founder-call/founder-session/allocation intents,
 *                            and as the primary CTA on the FounderVideo
 *                            surface while the video is in production.
 *   · FounderVideo · v2    · cinematic letterbox frame, corner ticks,
 *                            timecode strip, direct book-a-call when env URL
 *                            is wired. Designed placeholder, not a broken
 *                            player, when VIDEO_URL is unset.
 *   · Admin dashboard      · hidden /_/pipeline surface, token-gated,
 *                            no-indexed, renders the latest 100 Airtable
 *                            leads with score buckets and mailto links.
 *   · Copy tighten         · HardCTA tiers, INTENT_COPY modal copy, and CTA
 *                            verbs pulled to "Take your seat", "Hold a seat
 *                            for later", "Thirty minutes. On camera."
 *
 * Chapters (unchanged):
 ...
 * HubSpot + Airtable + Resend when env vars are present. Admin dashboard
 * reads only from Airtable. No PHI, no userId, no cross-session memory.
 */
      ...
      >
        <Container width="wide">
          ...
          >
            ...
            <span className="text-white/55">It's an invitation.</span>
          </h2>
          ...
          <div className="mt-20 border-t border-white/10 pt-14">
            ...
            <p className="mt-4 max-w-2xl font-body text-[16px] md:text-[17px] leading-[1.55] text-white/70">
              Every path reaches the founder. Every reply comes from the founder.
              Confirmation hits your inbox inside a minute. The real reply lands inside a business day.
            </p>
            <div className="mt-10">
              <HardCTAWrapper />
            </div>
