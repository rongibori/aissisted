  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      {(!hasProfile || latestBiomarkers.length === 0 || !currentProtocol) && (
        <div className="mb-6 p-4 bg-surface-2 border border-line rounded-xl">
          <p className="text-sm font-medium text-signal mb-3">
              ...
              <div key={step.label} className="flex items-center gap-3">
                ...
                <span
                  className={`text-sm flex-1 ${
                    step.done ? "line-through text-muted" : "text-ink"
                  }`}
                >
                  {step.label}
                </span>
                {!step.done && (
                  <button
                    onClick={step.action}
                    className="text-xs text-signal hover:text-signal/80 shrink-0"
                  >
                    {step.cta} →
                  </button>
                )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <>
                ...
                <div className="flex flex-col gap-3">
                  {currentProtocol.recommendations.map((rec) => (
                    ...
                    >
                      <div className="w-2 h-2 mt-1.5 rounded-full bg-signal shrink-0" />
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted">{rec.timing}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-ink">Biomarkers</h2>
              <Button
                variant="ghost"
                size="sm"
              ...
              <div className="flex flex-col gap-2">
                {latestBiomarkers.slice(0, 6).map((b) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-muted capitalize truncate mr-2">
                        {b.name.replace(/_/g, " ")}
                      </span>
                      <div className="flex items-center gap-1.5 shrink-0">
                        {sparklines[b.name] && (
                          <Sparkline
                            ...
                            height={20}
                            color={
                              status === "optimal" ? "var(--ok)"
                              : status === "high" || status === "low" ? "var(--warn)"
                              : "var(--data)"
                            }
                          />
                        )}
