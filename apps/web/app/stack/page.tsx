          ...
          <div>
            ...
            {currentProtocol && (
              <p className="text-xs text-graphite-soft mt-1">
                Generated{" "}
                <span className="font-data">{new Date(currentProtocol.createdAt).toLocaleDateString()}</span>
              </p>
            )}
          </div>
          ...
          <>
            ...
            <Card className="mb-5">
              ...
              {currentProtocol.warnings.length > 0 && (
                <div className="mt-3 p-3 bg-warn/10 border border-warn/30 rounded-lg">
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col items-center gap-1 shrink-0">
                      <div className="w-10 h-10 rounded-full bg-midnight/5 border border-midnight/20 flex items-center justify-center">
                        <span className="text-xs font-bold text-midnight font-data">
                          {Math.round(rec.score * 100)}
                        </span>
                      </div>
                      <span className="text-[10px] text-graphite-soft">score</span>
                    </div>
                    ...
                    <span className="text-graphite-soft text-sm shrink-0">
                      {expanded === rec.id ? "▲" : "▼"}
                    </span>
                  </div>
                </Card>
            ...
            {history.length > 1 && (
              <div className="mt-6">
                <button
                  ...
                  className="text-sm text-graphite-soft hover:text-graphite transition-colors mb-3 flex items-center gap-1"
                >
                ...
                {showHistory && (
                  <div className="flex flex-col gap-2">
                    {history.slice(1).map((p) => (
                      ...
                      >
                        <p className="text-xs text-graphite-soft mb-1">
                        ...
                        <p className="text-sm text-graphite leading-relaxed line-clamp-2">
                          {p.summary}
                        </p>
                      </div>
                    ))}
            ...
            <p className="text-xs text-center text-graphite-soft mt-6">
