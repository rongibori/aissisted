...
If the Postgres migration fails mid-run, or post-migration validation reveals a data-integrity regression, **do not retry the migration script on a partially-migrated database.** Restore from the pre-migration RDS snapshot.

### Prerequisites

- A manual RDS snapshot was taken immediately before running `apply-postgres-migration.sh` (required by the runbook).
- Snapshot identifier is recorded in the migration run log (e.g., `aissisted-postgres-pre-migration-YYYYMMDD-HHMM`).
- AWS CLI configured with credentials that have `rds:RestoreDBInstanceFromDBSnapshot` permission.

### Rollback procedure

1. Stop all API traffic to the current RDS instance (pause ECS service or update security group to deny app SG).
2. Restore the snapshot to a new DB instance:
```bash
   aws rds restore-db-instance-from-db-snapshot \
     --db-instance-identifier aissisted-postgres-recovered \
     ...
     --region us-east-1
```
3. Wait for status `available` (typically 10–20 minutes):
```bash
   aws rds wait db-instance-available \
     --db-instance-identifier aissisted-postgres-recovered \
     --region us-east-1
```
4. Rotate the `DATABASE_URL` parameter in SSM Parameter Store to point to the recovered instance endpoint.
5. Restart the ECS service. Validate API health at `/health`.
6. Once traffic is confirmed stable on the recovered instance, schedule teardown of the failed original instance (retain its snapshot for post-mortem).

### Post-rollback

- Open a post-mortem issue documenting: what the migration script did before failing, what data-integrity checks failed, what needs to change in the script before the next attempt.
- Do not re-run the migration until the post-mortem is resolved and the script has been updated.
